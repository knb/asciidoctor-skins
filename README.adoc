...
This repository is a fork of https://github.com/darshandsoni/asciidoctor-skins[asciidoctor-skins], extended with a link:skin-maker/README.adoc[Skin Maker] toolchain and hosted on GitHub Pages for convenience.
...
* **Skin preview demo:** https://larento.github.io/asciidoctor-skins/[larento.github.io/asciidoctor-skins]
* **Skin Maker editor:** https://larento.github.io/asciidoctor-skins/skin-maker/editor/index.html[Open the visual editor]

The site is deployed from the `main` branch. On each push, CI runs `bundle exec rake editor:prepare` to generate preview HTML and source maps before publishing.

NOTE: This is a project site served under the `/asciidoctor-skins/` path prefix. Use the links above rather than assuming the repository root URL.

== Skin Maker visual editor

The visual editor lets you pick a layout, tune colors and fonts, preview the result, and export a theme YAML file. It is the fastest way to try Skin Maker without editing YAML by hand.

**Open on GitHub Pages:** https://larento.github.io/asciidoctor-skins/skin-maker/editor/index.html[Skin Maker editor]
...
* Source inspect: click a preview element to see the matching AsciiDoc source
* Download theme YAML
* Copy a CSS custom-properties block to the clipboard
* Download compiled CSS when running the local server (see below)

=== Using the editor on GitHub Pages

On GitHub Pages you can use live preview, source inspect, YAML export, and CSS variable copy.

WARNING: GitHub Pages cannot run the Sass compile API. **CSS download is not available online.** Download YAML from the editor, then build CSS locally (see <<local-build>>) or run the local server.

=== Running the editor locally

For full functionality, including CSS download, start the bundled WEBrick server:
...
`editor:open` generates editor assets, renders preview HTML, starts the server on port `8765`, and opens the browser.
...
# open http://127.0.0.1:8765/skin-maker/editor/index.html
----

CAUTION: Opening `skin-maker/editor/index.html` directly with `file://` (for example `xdg-open skin-maker/editor/index.html`) disables live preview. Browsers block `fetch` and iframe access on local files. Color editing and YAML export still work, but preview and source inspect require HTTP.

[[local-build]]
=== After exporting a theme

Save the downloaded YAML under `themes/`, then generate CSS:
...
See link:skin-maker/README.adoc[skin-maker/README.adoc] for theme YAML format, CLI usage, and design references.
...
You can preview all of the stylesheets in the https://darshandsoni.com/asciidoctor-skins[online demo] (upstream) or on https://larento.github.io/asciidoctor-skins/[GitHub Pages] (this fork).
...
This repository includes a link:skin-maker/README.adoc[Ruby + Sass skin maker] that separates *layout* (structure) from *theme* (colors and fonts).
...
Generated CSS is written to `css/`. Use the link:https://larento.github.io/asciidoctor-skins/skin-maker/editor/index.html[visual editor] for interactive tuning, or edit YAML directly.
